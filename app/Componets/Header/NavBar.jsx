...
function NavBar() {
    ...
    return (
        <div className='h-22 bg-black w-full  overflow-hidden'>
            ...
            <nav className={`fixed  trans md:top-0 -bottom-2 items-center md:justify-evenly justify-center w-full flex md:flex-row  gap-4 md:gap-0 ${showMobileMenu ? 'h-16 scale-100 ' : showCart ? '' : 'h-0 p-0 '} ${showCart ? 'h-16 scale-100 w-[50%] left-[22rem] ' : 'w-[100%] left-[0rem] '}  rounded-t-2xl md:rounded-none  bg-black-900 text-white md:text-black md:bg-white group   md:h-8 z-[99999]`}>
                ...
                <button onClick={toggleCart} className={`trans bg-black rounded-full p-2 center  gap-4 absolute flex -top-[3.7rem] ${showCart ? 'right-80' : 'right-2'}`}>
